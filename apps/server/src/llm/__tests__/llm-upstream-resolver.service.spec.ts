  ...
  const mock = {
    ...
    llmModel: {
      findMany: vi.fn().mockImplementation(async ({ where }) => {
        if (where.modelId !== DEFAULT_LLM_DIGEST_MODEL_ID) {
          return [];
        }
        return [
          {
            id: 'm1',
            upstreamId: DEFAULT_LLM_DIGEST_MODEL_ID,
            modelId: DEFAULT_LLM_DIGEST_MODEL_ID,
            displayName: 'GPT-4o',
            inputTokenPrice: 0,
            outputTokenPrice: 0,
            minTier: 'FREE',
...
describe('LlmUpstreamResolverService', () => {
  ...
  it('creates settings defaults when missing and resolves upstream', async () => {
    ...
    const result = await service.resolveUpstream({ purpose: 'digest' });
    ...
    expect((prisma as any).llmModel.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ modelId: DEFAULT_LLM_DIGEST_MODEL_ID }),
      }),
    );
    expect(result.upstreamModelId).toBe(DEFAULT_LLM_DIGEST_MODEL_ID);
