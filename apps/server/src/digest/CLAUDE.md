...
[POS]: `apps/server/src/digest/*` 的统一入口说明（供人类与 AI 协作）
...
Digest 是 Anyhunt 的核心产品域：订阅 → 抓取/筛选 → 生成摘要/叙事 → 投递到 Inbox/Webhook/Email。
...
- 默认模型使用 `LlmSettings.defaultDigestModelId`；不在 Digest 内硬编码 model。
