...
export class SubscriptionRunProcessor extends WorkerHost {
  ...
  async process(job: Job<DigestSubscriptionRunJobData>) {
    ...
    try {
      ...
      totalCredits +=
        billingBreakdown['digest.acquire.search']?.subtotalCredits || 0;
      ...
      totalCredits +=
        billingBreakdown['digest.acquire.scrape']?.subtotalCredits || 0;
  ...
  > {
    ...
    // 记录计费
    billingBreakdown['digest.acquire.search'] = {
      count: 1,
      costPerCall: BILLING.costs['digest.acquire.search'],
      subtotalCredits: BILLING.costs['digest.acquire.search'],
    };
  ...
  > {
    ...
    if (scrapeCount > 0) {
      billingBreakdown['digest.acquire.scrape'] = {
        count: scrapeCount,
        costPerCall: BILLING.costs['digest.acquire.scrape'],
        subtotalCredits: scrapeCount * BILLING.costs['digest.acquire.scrape'],
      };
    }
